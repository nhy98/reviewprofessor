...
class Reviews extends Component {
  ...
  componentDidMount() {
     ...
     .then(data => this.setState({ isloaded: true, reviews: data.data.content, isnull: data.data.totalElements}))
    .catch((error) => {
      console.log(error);
    });
  }
  ...
  render() {
    var {  isloaded, reviews, isnull } = this.state;
    var rv = reviews[this.state.current_user];
    if (!isloaded) {
        return (
        ...
        if (isnull === 0) {
            return (
                <h2 className="mt-4 text-center text-white">This page currently has no review!</h2>
            )
        }
        else {
        return (
        ...
        centered>
            ...
            <h2 className="mt-4 text-white text-center">Recent reviews</h2>
